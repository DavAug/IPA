...
# fix growth rate
name = np.array(['central_compartment.lambda'])
value = np.array([2.0])
linear_model.fix_model_dof(names=name, values=value)

# set true values of dof to optimise
true_init_cond = [1]
parameter_boundaries = [[0, 100]]

# instantiate IP
profiler = ip.informationProfiler(model=linear_model,
                                  parameters=true_init_cond,
